...
public class RpcClient {
    ...
    private void sendRequest(String request) throws IOException {
        if (BuildConfig.DEBUG) Log.d(TAG, "mRequest.capacity() = " + mRequest.capacity());
        if (BuildConfig.DEBUG) Log.d(TAG, "Sending request: \n" + request.toString());
    ...
    private String receiveReply() throws IOException {
        mResult.setLength(0);
        if (BuildConfig.DEBUG) Log.d(TAG, "mResult.capacity() = " + mResult.capacity());

        long readStart = 0;
        if (BuildConfig.DEBUG) readStart = System.nanoTime();
